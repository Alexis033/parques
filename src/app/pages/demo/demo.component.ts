...
import { BOARD_LAYOUT, getCoordinates } from '@parchis/engine';
import type { SquareInfo, BoardPosition, PlayerColor, DiceRoll, Player, Room, HouseRules } from '@parchis/shared';
      ...
      <div class="demo-grid">
        ...
        <section class="demo-section">
          ...
          <div class="board-wrapper">
            ...
            />
            @for (token of mockTokens(); track token.id) {
              <app-token
                [token]="token"
                [isSelected]="token.id === 'r1'"
                [isValidMove]="token.id === 'r2'"
...
export class DemoComponent {
  ...
  mockTokens = signal<EngineToken[]>([
    ...
    { id: 'b1', color: 'BLUE', position: 10, state: 'IN_TRANSIT', totalSteps: 10, index: 0 },
    { id: 'b2', color: 'BLUE', position: -1, state: 'JAIL', totalSteps: 0, index: 1 },
    { id: 'g1', color: 'GREEN', position: 70, state: 'IN_SKY', totalSteps: 86, index: 0 },
  ...
  getStackIndex(token: EngineToken): number {
    const samePos = this.mockTokens().filter(t => t.position === token.position && t.id !== token.id);
